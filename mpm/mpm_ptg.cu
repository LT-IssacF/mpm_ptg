...
void init_solver(float *&x_p, const std::vector<float>& h_x_p, float *&v_p, float *&F_p, float *&B_p, float *&m_i, float *&v_i, float *&f_i, p2g_buffer *&buffer, int h_num_p, int h_num_g);
void substep(float *x_p, float *v_p, float *F_p, float *B_p, float *m_i, float *v_i, float *f_i, p2g_buffer *buffer, int h_num_p, int h_num_g);
...
int main() {
    ...
    float *f_i;
    p2g_buffer *buffer;
    init_solver(x_p, h_x_p, v_p, F_p, B_p, m_i, v_i, f_i, buffer, h_num_p, num_grid_cells);
    ...
    for (int frame = 0; frame < h_num_steps; frame++) {
        ...
        for (int s = 0; s < h_substeps_per_step; s++) {
            substep(x_p, v_p, F_p, B_p, m_i, v_i, f_i, buffer, h_num_p, num_grid_cells);
        ...
        export_to_obj("res/ptg/res_" + std::to_string(frame + 1) + ".obj", x_p, h_num_p);
    ...
    // 将统计信息写入 JSON 文件
    {
        std::ofstream json_file("res/ptg/timings_ptg.json");
        if (json_file.is_open()) {
            ...
            json_file << "}\n";
            json_file.close();
            std::cout << "Timings written to res/ptg/timings.json" << std::endl;
        } else {
            std::cerr << "Failed to open res/ptg/timings.json for writing" << std::endl;
        }
    }
...
__global__ void p2g_step1(float *particles_position, float *particles_velocity, float 
*particles_deformation_gradient, float *B_p, p2g_buffer *buffer) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < num_p) {
        float x_p[3];
        x_p[0] = particles_position[idx * 3 + 0];
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                for (int k = 0; k < 3; k++) {
                    ...
                    float m_i_contrib=m_p*weight; 
                    float v_i_contrib[3];
                    v_i_contrib[0]=m_p * weight * (v_p[0] + (C_p[0] * x_ip[0] + C_p[3] * x_ip[1] + C_p[6] * x_ip[2]));
                    v_i_contrib[1]=m_p * weight * (v_p[1] + (C_p[1] * x_ip[0] + C_p[4] * x_ip[1] + C_p[7] * x_ip[2]));
                    v_i_contrib[2]=m_p * weight * (v_p[2] + (C_p[2] * x_ip[0] + C_p[5] * x_ip[1] + C_p[8] * x_ip[2]));
                    float f_i_contrib[3];
                    f_i_contrib[0]=-(contribution[0] * weight_gradient[0] + contribution[3] * weight_gradient[1] + contribution[6] * weight_gradient[2]);
                    f_i_contrib[1]=-(contribution[1] * weight_gradient[0] + contribution[4] * weight_gradient[1] + contribution[7] * weight_gradient[2]);
                    f_i_contrib[2]=-(contribution[2] * weight_gradient[0] + contribution[5] * weight_gradient[1] + contribution[8] * weight_gradient[2]);

                    buffer[base+offset].grid_index=index; 
                    buffer[base+offset].m_i_contrib=m_i_contrib; 
                    buffer[base+offset].v_i_contrib[0]=v_i_contrib[0]; 
                    buffer[base+offset].v_i_contrib[1]=v_i_contrib[1]; 
                    buffer[base+offset].v_i_contrib[2]=v_i_contrib[2]; 
                    buffer[base+offset].f_i_contrib[0]=f_i_contrib[0]; 
                    buffer[base+offset].f_i_contrib[1]=f_i_contrib[1];  
                    buffer[base+offset].f_i_contrib[2]=f_i_contrib[2]; 
                    offset++;
                }
            }
        }
...
__global__ void p2g_reduce_atomic(p2g_buffer* buffer, float* m_i, float* v_i, float* 
f_i) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= (num_p*27)) return;
    p2g_buffer& entry=buffer[idx];
    int grid_idx=entry.grid_index;
    atomicAdd(m_i+grid_idx,entry.m_i_contrib);
    atomicAdd(v_i + grid_idx * 3 + 0, entry.v_i_contrib[0]);
    atomicAdd(v_i + grid_idx * 3 + 1, entry.v_i_contrib[1]);
    atomicAdd(v_i + grid_idx * 3 + 2, entry.v_i_contrib[2]);
    atomicAdd(f_i + grid_idx * 3 + 0, entry.f_i_contrib[0]);
    atomicAdd(f_i + grid_idx * 3 + 1, entry.f_i_contrib[1]);
    atomicAdd(f_i + grid_idx * 3 + 2, entry.f_i_contrib[2]);
}
...
void init_solver(float *&x_p, const std::vector<float>& h_x_p, float *&v_p, float *&F_p, float *&B_p, float *&m_i, float *&v_i, float *&f_i, p2g_buffer *&buffer, int h_num_p, int h_num_g) {
    ...
    cudaMalloc(&buffer,h_num_p * 27 * sizeof(p2g_buffer));
}
...
void substep(float *x_p, float *v_p, float *F_p, float *B_p, float *m_i, float *v_i, float *f_i, p2g_buffer *buffer, int h_num_p, int h_num_g) {
    ...
    p2g_step1<<<(h_num_p + 255) / 256, 256>>>(x_p, v_p, F_p, B_p, buffer);
    p2g_reduce_atomic<<<((h_num_p * 27) + 255) / 256, 256>>>(buffer, m_i, v_i, f_i);
